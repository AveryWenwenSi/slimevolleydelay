...
class SlimeVolleyEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.delay_step = 3
    self.action_buffer0 = [np.zeros([3])] * self.delay_step
    self.action_buffer1 = [np.zeros([3])] * self.delay_step
  ...
  def step(self, action, otherAction=None):
    ...
    #import pdb
    queue_step = self.t % self.delay_step

    #pdb.set_trace()
    self.game.agent_left.setAction(self.action_buffer1[queue_step])  #otherAction
    self.game.agent_right.setAction(self.action_buffer0[queue_step]) # action external agent is agent_right

    self.action_buffer0[queue_step] = action
    self.action_buffer1[queue_step] = otherAction
    ...
    if done:
      print("self_play delay=", self.delay_step)
